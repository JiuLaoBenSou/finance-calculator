...
async function main() {
  ...
  // Test with 20 codes starting from index 100 (likely stocks, not indices)
  const testCodes = codes.slice(100, 120);
  console.log('Testing:', testCodes);

  const results = await Promise.all(testCodes.map(c => getStockData(c)));

  for (const result of results) {
    console.log(result.code + ': name=' + result.name + ', klines=' + result.klines.length);
  }
}
