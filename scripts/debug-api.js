...
async function getStockData(code, days = 2500) {
  try {
    ...
    if (!response.data || !response.data[code]) {
      console.log(`No data for ${code}:`, response.msg);
    ...
    console.log(`No klines for ${code}, qfqday: ${stockData.qfqday ? stockData.qfqday.length : 0}, day: ${stockData.day ? stockData.day.length : 0}`);
    return { code, name, klines: [] };
  } catch (e) {
...
async function main() {
  ...
  const testCodes = ['sh600000', 'sh600001', 'sz000001', 'sz000002', 'sh000001'];
  console.log('Testing API with sample codes...');

  for (const code of testCodes) {
    const result = await getStockData(code);
    console.log(`${code}: name="${result.name}", klines=${result.klines.length}, latest=${result.klines[result.klines.length-1]?.date}`);
  }
}
