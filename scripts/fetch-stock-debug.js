/**
 * Stock data update script - debug version
 */
...
async function main() {
  ...
  console.log('Total files:', codes.length);
  ...
  for (const code of codes.slice(0, 5)) {
    ...
    console.log(code + ': name=' + result.name + ', klines=' + result.klines.length);
  }
  ...
  for (let i = 0; i < codes.length; i += CONCURRENCY) {
    ...
    console.log('Progress: ' + Math.min(i + CONCURRENCY, codes.length) + '/' + codes.length + ' (' + success + ' success)');
  }
